...
const LANGS = {
  ...
  es: { dir: "ltr", login: "Iniciar sesión", register: "Crear cuenta", email: "Correo", password: "Contraseña", send: "Enviar", logout: "Cerrar sesión", placeholder: "Pregunta lo que quieras...", noAccount: "¿No tienes cuenta? ", hasAccount: "¿Ya tienes cuenta? ", signUp: "Registrarse", signIn: "Entrar", welcome: "¡Hola! Soy ShardeumAI. ¿En qué puedo ayudarte?", thinking: "Pensando...", error: "Error al obtener respuesta. Inténtalo de nuevo." },
};
...
export default function App() {
  ...
  async function handleAuth(e) {
    ...
    try {
      if (isSignUp) {
        ...
        setAuthMsg("Registration successful! Please sign in.");
        setIsSignUp(false);
      } else {
      ...
      if (msg.includes("Invalid login")) setAuthMsg("Invalid email or password.");
      else if (msg.includes("already")) setAuthMsg("Email already registered.");
      else if (msg.includes("Password")) setAuthMsg("Password must be at least 6 characters.");
      else setAuthMsg(msg);
    } finally {
  ...
  if (!session) {
    return (
      <div style={{ ...S.center, background: "#0b0f14", flexDirection: "column", gap: 16 }}>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center" }}>
          {Object.keys(LANGS).map(l => (
            <button key={l} onClick={() => setLang(l)} style={{ ...S.langBtn, background: lang === l ? "#4fd1c5" : "#161c25", color: lang === l ? "#06201c" : "#8b96a3" }}>
              {l.toUpperCase()}
            </button>
          ))}
        </div>
        <div style={{ ...S.card, direction: t.dir }}>
          <div style={S.brandRow}>
            <div style={S.mark}>SD</div>
            <div>
              <div style={S.brandName}>ShardeumAI</div>
              <div style={S.brandSub}>SDAI · Intelligent Assistant</div>
            </div>
          </div>
          <div style={S.tabs}>
            <button onClick={() => { setIsSignUp(false); setAuthMsg(""); }} style={{ ...S.tab, ...(!isSignUp ? S.tabActive : {}) }}>{t.login}</button>
            <button onClick={() => { setIsSignUp(true); setAuthMsg(""); }} style={{ ...S.tab, ...(isSignUp ? S.tabActive : {}) }}>{t.register}</button>
          </div>
          <form onSubmit={handleAuth} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={S.field}>
              <label style={S.label}>{t.email}</label>
              <input type="email" required value={email} onChange={e => setEmail(e.target.value)} style={S.input} placeholder="you@example.com" dir="ltr" />
            </div>
            <div style={S.field}>
              <label style={S.label}>{t.password}</label>
              <input type="password" required minLength={6} value={password} onChange={e => setPassword(e.target.value)} style={S.input} placeholder="••••••••" dir="ltr" />
            </div>
            {authMsg && <div style={{ fontSize: 13, color: authMsg.includes("successful") ? "#4fd1c5" : "#e0746a", padding: "8px 12px", background: "rgba(79,209,197,0.08)", borderRadius: 8 }}>{authMsg}</div>}
            <button type="submit" disabled={authLoading} style={S.submitBtn}>{authLoading ? "..." : isSignUp ? t.register : t.login}</button>
          </form>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: "#0b0f14", color: "#e8edf2", fontFamily: "Inter, sans-serif", direction: t.dir }}>
      <header style={S.header}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={S.headerMark}>SD</div>
          <div>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <select value={lang} onChange={e => setLang(e.target.value)} style={{ background: "#161c25", color: "#e8edf2", border: "1px solid #232b36", borderRadius: 8, padding: "6px 10px", fontSize: 13, cursor: "pointer" }}>
            {Object.keys(LANGS).map(l => <option key={l} value={l}>{l.toUpperCase()}</option>)}
          </select>
          <button onClick={() => supabase.auth.signOut()} style={S.logoutBtn}>{t.logout}</button>
        </div>
      </header>
      ...
      <div ref={scrollRef} style={S.msgArea}>
        {messages.length === 0 && (
          <div style={S.welcome}>
            <div style={S.welcomeMark}>SD</div>
            <p style={{ maxWidth: 420, textAlign: "center", fontSize: 14.5, color: "#8b96a3", lineHeight: 1.8 }}>{t.welcome}</p>
          </div>
        )}
        {messages.map((m, i) => (
        ...
        {chatLoading && (
          <div style={{ display: "flex", justifyContent: t.dir === "rtl" ? "flex-end" : "flex-start" }}>
            <div style={{ ...S.bubble, ...S.aiBubble, color: "#8b96a3", fontStyle: "italic" }}>{t.thinking}</div>
          </div>
        )}
      </div>

      <form onSubmit={handleSend} style={S.composer}>
        <input value={input} onChange={e => setInput(e.target.value)} placeholder={t.placeholder} style={S.composerInput} disabled={chatLoading} />
        <button type="submit" disabled={!input.trim() || chatLoading} style={S.sendBtn}>{t.send}</button>
      </form>
    </div>
  );
}
